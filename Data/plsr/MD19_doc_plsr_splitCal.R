...
################################################ RUN THIS SECTION FOR MAY-AUGUST CALIBRATION ################################################
...
# Display plot
p
ggsave("Data/plsr/figures/MD_doc_labvspred_may-aug.png")
...
### Plot time series ----------------------------------------------
 ...
 ###### plotting predicted DOC values by month
  ...
  t
  ggsave("Data/plsr/figures/MD_doc_plsr_output_may-aug_cal.png")

  write.csv(time_series, file = "Data/plsr/predicted_time_series/MD19_doc_predicted_time_series_4cmpt_may-aug_cal.csv", row.names = FALSE) # Writes the loadings to a file

  ################################################ RUN THIS SECTION FOR OCTOBER CALIBRATION ################################################
  # PULL OUT SUBSET OF DATA WE WANT TO WORK WITH; filter out very low values and rename NPOC to DOC - and removing any samples from October
  subset <- data %>% filter(!is.na(NPOC_mgCL)) %>% filter(NPOC_mgCL>0.03) %>% rename(DOC_mgL="NPOC_mgCL") %>% 
    filter(month==10)
  ...
  # Add point geom
  p <- p+ geom_point(mapping = aes(color = r_timestamp), size = 2)
  p <- p+ geom_smooth(method = lm, color = "black") + xlim(4.5,6) + ylim(4.5,6)
  # Label axes
  p <- p+ labs(x = paste("Lab", param, "value"),
               y = paste ("Predicted", param, "value -", type, "fp") )
  # Change text to bold, a litter bigger
