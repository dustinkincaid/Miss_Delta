...
# PLSR NO3 calibration script for Miss Delta samples 
...
# PULL OUT SUBSET OF DATA WE WANT TO WORK WITH
subset <- data %>% filter(!is.na(NO3_mgNL))

### plotting boxplot of samples to see range of samples in calibration library 
MD_no3_dist <- boxplot(subset$NO3_mgNL~subset$month,horizontal=TRUE,main="Miss Delta cal. library NO3 distribution",xlab="NO3 mg N/L",ylab="Month")
MD_no3_dist
...
# Start the plot
#p <- plot(fit_results$lab_val,fit_results$predicted)
...
# Display plot
p
ggsave("Data/plsr/figures/MD_no3_labvspred.png")
...
#  sensor_data1 <- data %>%  select(timestamp_chem:turb_NTU_scan & comp_)
 ...
 ### Generate time series ----------------------------------------------------
  ...
  cal_lib_attributes <- data %>% select(timestamp_chem:NPOC_mgCL) 
...
### Plot time series ----------------------------------------------
 ...
 ###### plotting predicted NO3 values by month

  t <- ggplot(data=time_series) + geom_point(aes(x=r_timestamp,y=predicted_param),color="black")
  t <- t + geom_point(aes(x=r_timestamp,y=NO3_mgNL,color="red"))
  ...
  t
  ggsave("Data/plsr/figures/MD_no3_plsr_output.png")
      

  write.csv(time_series, file = "Data/plsr/predicted_time_series/MD19_no3_predicted_time_series_5cmpt.csv", row.names = FALSE) # Writes the loadings to a file
