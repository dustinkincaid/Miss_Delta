...
# PULL OUT SUBSET OF DATA WE WANT TO WORK WITH; filter out very low values and rename NPOC to DOC 
...
p <- p+ geom_smooth(method = lm, color = "black")+ xlim(2.5,6) + ylim(2.5,6)
# Label axes
...
# Display plot
p
ggsave("Data/plsr/figures/MD_doc_labvspred.png")
...
### Plot time series ----------------------------------------------
 ...
 ###### plotting predicted DOC values by month
  ...
  write.csv(time_series, file = "Data/plsr/predicted_time_series/MD19_doc_predicted_time_series_5cmpt.csv", row.names = FALSE) # Writes the loadings to a file
