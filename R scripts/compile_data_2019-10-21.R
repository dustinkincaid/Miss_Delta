...
# from Missisquoi River Delta longitudinal survey on 2019-10-21 into one file
...
# Read in files
  ...
  # Change file name
  gps <- read_csv("Data/GPS Data/survey_distances_2019-10-21.csv", col_types = cols()) %>%
    # Convert timestamp
  ...
  ysi <- read_csv("Data/YSI EXO2 Data/YSI_EXO2_Miss_Delta_20191021.csv", col_types = cols()) %>% 
    ...
    select(Date, Time, r_timestamp, "Temp C", "ODO % sat", "ODO mg/L", "SpCond uS/cm", "TDS mg/L", "pH", "Chlorophyll ug/L", "fDOM QSU") %>% 
    # Rename columns
    rename(temp_c = "Temp C", do_per = "ODO % sat", do_mgl = "ODO mg/L", spc_uscm = "SpCond uS/cm",
           tds_mgl = "TDS mg/L", ph = "pH", chla_ugl = "Chlorophyll ug/L", fdom_qsu = "fDOM QSU") %>% 
    # Filter readings to only include those during formal survey period
    filter((r_timestamp >= ymd_hms("2019-10-21 10:06:00", tz = "America/New_York") & r_timestamp <= ymd_hms("2019-10-21 11:24:00", tz = "America/New_York")) |
             (r_timestamp >= ymd_hms("2019-10-21 12:05:00", tz = "America/New_York") & r_timestamp <= ymd_hms("2019-10-21 14:24:00", tz = "America/New_York"))) %>% 
    # Add a column to indicate which transect the readings belong to: Dead Creek Run vs. mainstem
    # mutate(transect = ifelse(r_timestamp >= ymd_hms("2019-10-21 10:06:00", tz = "America/New_York") & r_timestamp <= ymd_hms("2019-10-21 11:24:00", tz = "America/New_York"),
    #                          "Dead Creek", "Main")) %>%
    # Rearrange columns
    select(Date, Time, r_timestamp, everything())
  
  
  # Grab sample solute chemistry
  # Dissolved nutrients
  chem_diss <- read_csv("Data/Lab Chemistry/MD19_Dissolved_set2_QC.csv") %>%
  ...
  chem_totDiss <- read_csv("Data/Lab Chemistry/MD19_TDN-TDP_set2_QC.csv", col_types = cols(Time = "c")) %>%
    # Format timestamp and make r_timestamp
    ...
    # Get rid of unnecessary columns
    select(-c(LabID, Date, Time))
  # Total nutrients
  chem_tot <- read_csv("Data/Lab Chemistry/MD19_TN-TP_set2_QC.csv", col_types = cols(Time = "c")) %>%
    # Format timestamp and make r_timestamp
    mutate(r_timestamp = mdy_hm(paste(Date, Time), tz = "America/New_York")) %>%
    # Rename columns
  ...
  # August only
  chem_oct <- chem %>% filter(month(r_timestamp) == 10) %>% select(r_timestamp, NO3_UM:TP_mgPL)
  ...
  # Keep only October DOC data
  doc_oct <- doc %>% filter(month(r_timestamp) == 10)
  
  
  # TSS
  tss <- read_csv("Data/TSS/TSS_2019-10-21.csv", col_types = cols()) %>% 
    # Create timestamp
  ...
  # Predicted chemistry from s::can global calibration
    ...
    # scan <- read_csv("Data/scan Data/2019-10-21 scan data/scan_globalcalibration_2019-10-21.csv", col_types = cols()) %>%
    #   # Rename columns
    ...
    scan_no3 <- read_csv("Data/plsr/predicted_time_series/MD19_no3_predicted_time_series_5cmpt.csv") %>% 
      ...
      mutate(r_timestamp = ymd_hms(r_timestamp, tz = "America/New_York")) %>% 
      # Keep only this month's data
      filter(month(r_timestamp) == 10)
    scan_doc <- read_csv("Data/plsr/predicted_time_series/MD19_doc_predicted_time_series_3cmpt_oct_cal.csv") %>% 
      select(timestamp_chem, predicted_param) %>% 
  ...
  alldata <- 
    ...
    # remove rows with no gps data i.e. when we pulled over at the boat launch
    filter(!is.na(lat)) %>%     
    # Add the nutrient data
    full_join(chem_oct, by = "r_timestamp") %>% 
    # Add the tss data
    full_join(tss %>% select(-c(Date, Time)), by = "r_timestamp") %>%
    # Add the DOC 
    full_join(doc_oct, by = "r_timestamp") %>% 
...
# Write the alldata file as CSV
  alldata %>% 
    ...
    # Write as CSV
    write_csv("Data/alldata_compiled_2019-10-21.csv")
