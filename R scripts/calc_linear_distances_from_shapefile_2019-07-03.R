...
# so that 0 meters is the same 0 meters as the first survey on 2019-05-17
...
# 2019-07-03 float data
  md_sf <- st_read("Data/GPS Data/2019-07-03 GPS Data/Export_5_3_Data.shp") %>% 
    # Convert timestamp
    mutate(r_timestamp = ymd_hms(DateTimeS, tz = "America/New_York")) %>% 
    # Filter out rows before beginning of survey (9:24)
...
# What is the linear distance offset b/w the first GPS points from the 2019-05-17 & 2019-07-03 floats?
# Calculate the linear distance b/w these two points
  ...
  first_july <- md_sf %>% slice(1) %>% select(r_timestamp, geometry)
  ...
  # Calculate the distance between these two points
    ...
    dist_offset <- rbind(first_july2, first_may) %>%
        mutate(
...
# Write file as a CSV
  write_csv(output, "Data/GPS Data/survey_distances_2019-07-03.csv")
