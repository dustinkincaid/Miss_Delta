...
# from Missisquoi River Delta longitudinal survey on 2019-05-17 into one file
...
# Read in files
  ...
  gps <- read_csv("Data/GPS Data/survey_distances_2019-05-17.csv", col_types = cols()) %>%
    # Convert timestamp
    mutate(r_timestamp = mdy_hm(r_timestamp, tz = "America/New_York"))
    
  
  # YSI EXO2 data
  ysi <- read_csv("Data/YSI EXO2 Data/YSI_EXO2_Miss_Delta_20190517.csv", col_types = cols()) %>% 
    ...
    select(Date, Time, r_timestamp, "C-15F102377", "DO %-15F101915", "DO mg/L-15F101915", "SPC-uS/cm-15F102377",
           "TDS mg/L-15F102377", "pH-18A105002", "Chl ug/L-15F101336", "fDOM QSU-13M100089") %>% 
    # Rename columns
    rename(temp_c = "C-15F102377", do_per = "DO %-15F101915", do_mgl = "DO mg/L-15F101915", spc_uscm = "SPC-uS/cm-15F102377",
           tds_mgl = "TDS mg/L-15F102377", ph = "pH-18A105002", chla_ugl = "Chl ug/L-15F101336", fdom_qsu = "fDOM QSU-13M100089") %>% 
    # Filter readings to only include those during formal survey period
    filter(r_timestamp >= ymd_hms("2019-05-17 09:24:00", tz = "America/New_York") & r_timestamp <= ymd_hms("2019-05-17 11:36:00", tz = "America/New_York"))
  ...
  # May only
  chem_may <- chem %>% filter(month(r_timestamp) == 5) %>% select(r_timestamp, NO3_UM:TP_mgPL)
  ...
  # Keep only May DOC data
  doc_may <- doc %>% filter(month(r_timestamp) == 5)
  ...
  tss <- read_csv("Data/TSS/TSS_2019-07-19.csv", col_types = cols()) %>% 
    ...
    #Omit high outlier for TSS in May
    mutate(tss_mgL = replace(tss_mgL, tss_mgL > 25, NA))
  ...
  # Predicted chemistry from s::can
    # Global calibration predictions
    # scan <- read_csv("Data/scan Data/2019-05-17 scan data/scan_globalcalibration_20190517.csv", col_types = cols()) %>%
    #   # Rename columns
    ...
    scan_no3 <- read_csv("Data/plsr/predicted_time_series/MD19_no3_predicted_time_series_5cmpt.csv") %>% 
      ...
      # Keep only this month's data
      filter(month(r_timestamp) == 5)
    # DOC
    scan_doc <- read_csv("Data/plsr/predicted_time_series/MD19_doc_predicted_time_series_1cmpt_may_cal.csv") %>% 
      select(timestamp_chem, predicted_param) %>% 
  ...
  alldata <- 
    ...
    full_join(chem_may, by = "r_timestamp") %>% 
    # Add the tss data
    full_join(tss %>% select(-c(Date, Time)), by = "r_timestamp") %>%
    # Add the DOC 
    full_join(doc_may, by = "r_timestamp") %>% 
...
# Write the alldata file as CSV
  alldata %>% 
    ...
    # Write as CSV
    write_csv("Data/alldata_compiled_2019-05-17.csv")
