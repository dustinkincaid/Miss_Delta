...
# The 8/14 & 10/21 floats started with a transect down Dead Run Creek
...
# 2019-08-14 float data
  md_sf_aug <- st_read("Data/GPS Data/2019-08-14 GPS Data/MissDelta_8-14_gps_points.shp") %>% 
    ...
    mutate(transect = ifelse(r_timestamp >= ymd_hms("2019-08-14 09:53:00", tz = "America/New_York") & r_timestamp <= ymd_hms("2019-08-14 11:10:00", tz = "America/New_York"),
                             "Dead Creek", "Main"))
# 2019-10-21 float data
  md_sf_oct <- st_read("Data/GPS Data/2019-10-21 GPS Data/GPS1021.shp") %>% 
    # Convert timestamp
    mutate(r_timestamp = ymd_hms(DateTimeS, tz = "America/New_York")) %>% 
    # Filter readings to only include those during formal survey period
...
# Split md_sf_oct into the two transects
  md_sf_oct_dcr <- md_sf_oct %>% filter(transect == "Dead Creek")
  md_sf_oct_main <- md_sf_oct %>% filter(transect == "Main")

# Dead Creek Run
  # Pull the first points from both surveys
  first_dcr_aug <- md_sf_aug_dcr %>% slice(1) %>% select(r_timestamp, geometry)
  first_dcr_oct <- md_sf_oct_dcr %>% slice(1) %>% select(r_timestamp, geometry)
  
  # In Oct, we started upstream of the starting point on DCR in Aug
  # I mapped these GPS points and then used ImageJ to calculate how far upstream we were in Oct
  # We were ~10.5 meters upstream (so need to SUBTRACT this value below)
  offset_value_dcr <- 10.5
...
# Calculate the linear distance b/w these two points
  ...
  first_oct <- md_sf_aug_main %>% slice(1) %>% select(r_timestamp, geometry)
  ...
  #       )
    ...
    # The October start point is basically the same as in May, so offset will be 0
    # Link to map of sites: https://batchgeo.com/map/7e687d3e1c40f818404f7577b43957f9
...
# Solution from here: https://github.com/r-spatial/sf/issues/799
  ...
  # Mainstem
  md_sf_oct_main2 <- md_sf_oct_main %>%
      mutate(
          dist_from_prev_m = sf::st_distance(
                geometry, 
                lag(geometry, default = empty), 
...
# Create a non-shapefile to output
  # Extract coordinates
  # Dead Creek Run
    md_coords_dcr <- do.call(rbind, st_geometry(md_sf_oct_dcr2)) %>%
      # Coerce the list to a data frame
      as_tibble() %>% 
      # Name columns
      setNames(c("lon", "lat", "elev"))
  # Mainstem
    md_coords_main <- do.call(rbind, st_geometry(md_sf_oct_main2)) %>%
      # Coerce the list to a data frame
      as_tibble() %>% 
      # Name columns
      setNames(c("lon", "lat", "elev"))    
  ...
  output_dcr <- 
    ...
    bind_cols(md_sf_oct_dcr2, md_coords_dcr) %>% 
    ...
    # CORRECT THE OFFSET, so that 0 meters is the same 0 meters as the first Dead Crk Run survey on 2019-08-14
    mutate(dist_cum_m = dist_cum_m - offset_value_dcr) %>% 
  ...
  output_main <- 
    # Add the extracted coordinates to the shapefile
    bind_cols(md_sf_oct_main2, md_coords_main) %>% 
    # Convert the distance column to numeric
    mutate(dist_from_prev_m = as.numeric(dist_from_prev_m)) %>%
    # Make the first distance a 0 instead of NA
    mutate(dist_from_prev_m = replace_na(dist_from_prev_m, 0)) %>% 
    ...
    mutate(dist_cum_m = dist_cum_m - offset_value_main) %>% 
    # Convert the r_timestamp to character to avoid issues with timezone conversions in CSV
    mutate(r_timestamp = as.character(r_timestamp)) %>% 
    # Select the columns to keep
    select(DateTimeS, r_timestamp, dist_from_prev_m, dist_cum_m, lat, lon, elev)  
...
# Write file as a CSV
  write_csv(output, "Data/GPS Data/survey_distances_2019-10-21.csv")
