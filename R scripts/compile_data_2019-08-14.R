...
# from Missisquoi River Delta longitudinal survey on 2019-08-14 into one file
...
# Read in files
  ...
  # Change file name
  gps <- read_csv("Data/GPS Data/survey_distances_2019-08-14.csv", col_types = cols()) %>%
    # Convert timestamp
  ...
  ysi <- read_csv("Data/YSI EXO2 Data/YSI_EXO2_Miss_Delta_20190814.csv", col_types = cols()) %>% 
  ...
  # Grab sample solute chemistry
    ...
    # August only
    chem_aug <- chem %>% filter(month(r_timestamp) == 8) %>% select(r_timestamp, NO3_UM:TP_mgPL)
    ...
    # Keep only August DOC data
    doc_aug <- doc %>% filter(month(r_timestamp) == 8)

  
  # TSS
  tss <- read_csv("Data/TSS/TSS_2019-08-14.csv", col_types = cols()) %>% 
    # Create timestamp
  ...
  # Predicted chemistry from s::can global calibration
    ...
    # scan <- read_csv("Data/scan Data/2019-08-14 scan data/scan_globalcalibration_20190814.csv", col_types = cols()) %>%
    #   # Rename columns
    ...
    scan_no3 <- read_csv("Data/plsr/predicted_time_series/MD19_no3_predicted_time_series_5cmpt.csv") %>% 
      ...
      mutate(r_timestamp = ymd_hms(r_timestamp, tz = "America/New_York")) %>% 
      # Keep only this month's data
      filter(month(r_timestamp) == 8)
    scan_doc <- read_csv("Data/plsr/predicted_time_series/MD19_doc_predicted_time_series_3cmpt_aug_cal.csv") %>% 
      select(timestamp_chem, predicted_param) %>% 
...
# Compile files into one file
  alldata <- 
    ...
    full_join(chem_aug, by = "r_timestamp") %>% 
    # Add the tss data
    full_join(tss %>% select(-c(Date, Time)), by = "r_timestamp") %>%
    # Add the DOC 
    full_join(doc_aug, by = "r_timestamp") %>% 
...
# Write the alldata file as CSV
  alldata %>% 
    ...
    # Write as CSV
    write_csv("Data/alldata_compiled_2019-08-14.csv")
